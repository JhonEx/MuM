...
public class Main {
    ...
    public static <T> boolean contains(List<T> list, T t) {
        return list.stream().filter(v -> v.equals(t)).findAny().isPresent();
    }

    public static <E> boolean contains(List<? extends E> list, E m, BiPredicate<? super E, ? super E> predicate) {
        return list.stream().filter(e -> predicate.test(e, m)).findAny().isPresent();
    }
    ...
    public static void test4() {
        ...
        Manager m = new Manager(1001, "Joe1", 50000, 500);
        boolean foundIt = Main.contains(list, m,
